...
onMounted(() => {
  ...
  // 断言contentEl为HTMLElement
  const el = document.getElementsByClassName(`${variables.namespace}-layout-content`)[0] as HTMLElement | null;
  Object.assign(contentEl, el)
  if (el) {
...
onBeforeUnmount(() => {
  ...
  const el = unref(contentEl) as HTMLElement | null;
  if (el) {
